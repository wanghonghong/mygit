 ...
 * 官方图文
 */
CommonUtils.regNamespace("op", "graphics");
op.graphics = (function(){
    var count = 3;
    var sumH5img = 0;
    var ajaxUrl = {
        url1:CONTEXT_PATH+'/image_text',//新增图文
        url2:CONTEXT_PATH+'/image_text_type'
    };
        ...
        $.ajaxJson(url,params,{
            "done":function(res){
                if(res.code===0){
                    ...
                    jumi.template('op/ph_list_img',data,function(tpl){
                        $('#table-body').empty();
                        $('#table-body').html(tpl);
                    })
                }
    ...
    var _bind = function(){
        ...
        $('#op_save').click(function () {
            ...
            if (form.valid()) {
                ...
                jumi.dialogSure('确定保存该项目图文的信息吗?',args);
            } else {
                $('body').animate({scrollTop:0},1000);
            }
        })
        ...
        $('input[name="image_showFormat"]').click(function(){
            ...
            var url = ajaxUrl.url2+'/saveShowFormat/'+value+'/type_id/1';
            $.ajaxJsonPut(url,'',{
                "done":function (res) {
                    if(res.code===0){
                        var dm = new dialogMessage({
        ...
        $('#op_tab').find('li').unbind('click').bind('click',function(){
            ...
            if(index === 1){
                ...
                var url = ajaxUrl.url2+'/list/type_id/1';
                var params = {
                    pageSize:10
                };
                jumi.pagination('#pageTypeToolbar',url,params,function(res,curPage){
                    if(res.code===0){
                        ...
                        jumi.template('op/ph_list_type',data,function(tpl){
            ...
            $.ajaxJsonGet(url,"",{
                "done":function (res) {
                    if(res.code===0){
                        ...
                        jumi.template('op/ph_edit_img',data,function(tpl){
                            var d = dialog({
                                title: '图文编辑',
                                content:tpl,
                                width:1200,
    ...
    var _initImageType = function(){
        jumi.template('op/index',function(html){
            ...
            var str =[];
            var url = ajaxUrl.url2+'/type_id/1';
