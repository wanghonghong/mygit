/**
 * Created by ray on 17/2/21.
 */

CommonUtils.regNamespace("juke", "code");
juke.code = (function() {
    var CodeNumber = 6;//聚客红包二维码海报总数
    var ajaxUrl = {
        url1:CONTEXT_PATH+'/qrcodePoster/save',//二维码海报保存
        url2:CONTEXT_PATH+'/qrcodePoster/jkdata',//二维码海报回填
        url3:CONTEXT_PATH+'/jk/jk_qrcode'//二维码下载
    }
    var _init = function(){
        _drag();
        _getCode();
    ...
    var _getJsonCode = function(){
        jumi.file('/juke/code_config.json',function(res){
            if(res.code===0){
                var data = {
                    items:res.data
                }
                jumi.template('jk/jk_code_list',data,function(tpl){
                    $('#code_list').html(tpl);
                    _setDrag(0,0);
    ...
    var _bind = function(){
        ...
        $('#activity_code_return').on('click',function(){
            jumi.template('jk/jk_manage_index',function(tpl){
                $('#shopIndexContent').empty();
                $('#shopIndexContent').html(tpl);
                juke.jukeRed.queryList(0);
                juke.jukeRed.bind();
            })
        })
        $('#activity_code_down').on('click',function(){
