CommonUtils.regNamespace("sales", "agent");

sales.agent=(function(){
    var feeType=2; //收费类型：1分销升级收费，2代理收费
    var opt={
    ...
    var _loadHtml=function () {
        ...
        opt.brokerageConfig.share=opt.brokerageConfig.share||'快来代理升级啊';
        _loadViewWindow();
        _initEditWindow();
    }
    var _loadViewWindow=function () {
        ...
        var data={
            ...
            instruction:opt.brokerageConfig.instruction||'代理说明描述',
            moneylist:moneylist
        }
    ...
    var  _getMoneyList=function () {
        var moneylist=[];
        if($("#settingArea [name='agentAfee']")){
            moneylist.push($("#settingArea [name='agentAfee']").val()||0);
        }
        if($("#settingArea [name='agentBfee']")){
            moneylist.push($("#settingArea [name='agentBfee']").val()||0);
        }
        if($("#settingArea [name='agentCfee']")){
            moneylist.push($("#settingArea [name='agentCfee']").val()||0);
        }
        if($("#settingArea [name='agentDfee']")){
            moneylist.push($("#settingArea [name='agentDfee']").val()||0);
        }
        return moneylist;
    }
